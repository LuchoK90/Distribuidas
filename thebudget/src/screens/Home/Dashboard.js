...
const HeadTable_2 = ['Tipo', 'Detalle', 'Fecha', 'Monto'];
  ...
  const select = () => {
    db.transaction((tx) => {
      ...
      tx.executeSql("select distinct rubro x, sum(monto) y from presupuestos where mes = '" + getMonth() + "' and anio = '" + getFullYear() + "' group by rubro", [], (_, { rows }) => {
