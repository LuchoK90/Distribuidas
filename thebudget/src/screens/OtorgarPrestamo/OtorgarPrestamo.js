...
const OtorgarPrestamo = ({ navigation }) => {
  ...
  const add=(medioCobro,monto,cantCuotas) => {
    ...
    db.transaction((tx) => {
      tx.executeSql(
        "insert into prestamos ( tipo , monto ,cuenta, cuotas ) values ('Otorgado',?,?,?)",
    ...
    db.transaction((tx) => {
      tx.executeSql(
        "insert into movimientos ( fecha, detalle, monto, medio, tipo_mov, comprobante) values (?,'Prest. Otorgado', ?, ?, 'Egreso', '')",
